...
<section class="section row">
            ...
            <div class="card-header">
                ...
                <div class="row">
                    ...
                    <div class="col-xl-12 col-md-12">
                        <button type="button" class="btn btn-sm btn-warning float-end" id="btnCari"><i class="bi bi-copy"></i></button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="col-sm-12 col-md-12">
        <div class="card ">
            <div class="card-header">
                <h4 class="card-title">History Permintaan</h4>
                <div class="col-xl-12 col-md-12">
                    <button type="" class="btn btn-warning float-end" id="btnCari"></button>
                </div>
            </div>
            <div class="card-body">
                <div class="table-responsive datatable-minimal">
...
<div class="modal fade text-left" id="modalAddPermintaan" role="dialog" aria-labelledby="myModalLabel140"
...
<div class="modal fade text-left" id="modalValidasiPermintaan" role="dialog" aria-labelledby="myModalLabel140"
    data-bs-backdrop="false">
    <div class="modal-dialog modal-full" role="document">
        <div class="modal-content">
            <div class="modal-header bg-info">
                ...
                <butto n type="button" class="close" data-bs-dismiss="modal" aria-label="Close">
                    <i data-feather="x"></i>
                    </button>
            </div>
            <div class="modal-body">
                <div class="row">
                    <div class="col-sm-8 col-md-6" hidden>
                        <div class="form-group">
                            <label for="basicInput">No. Perimtaaan</label>
                            <input type="text" class="form-control form-control-sm" id="VnoPerimtaaan" disabled>
                        </div>
                    </div>
                    <div class="col-sm-6 col-md-4">
                        <div class="form-group">
                            <label for="basicInput">Tgl. Perimtaaan</label>
                            <input type="date" class="form-control form-control-sm" id="VtglPerimtaaan"
                                value="{{ date('Y-m-d') }}" disabled>
                        </div>
                    </div>
                    <div class="col-sm-6 col-md-4" hidden>
                        <div class="form-group">
                            <label for="basicInput">kode Dari</label>
                            <input type="text" class="form-control" id="Vkdtujuan" disabled>
                        </div>
                    </div>
                    <div class="col-sm-6 col-md-4">
                        <div class="form-group">
                            <label for="basicInput">Dari</label>
                            <input type="text" class="form-control" id="Vtujuan" disabled>
                        </div>
                    </div>

                    <div class="col-sm-6 col-md-4" hidden>
                        <div class="form-group">
                            <label for="basicInput">kd Tujuan</label>
                            <input type="text" class="form-control" id="Vkddari" disabled>
                        </div>
                    </div>
                    <div class="col-sm-6 col-md-4">
                        <div class="form-group">
                            <label for="basicInput">Tujuan (Ke)</label>
                            <input type="text" class="form-control" id="Vdari" disabled>
                        </div>
                    </div>
                    ...
                    <div class="col-sm-8 col-md-12">
                        <div class="form-group">
                            <label for="basicInput">Keterangan</label>
                            <textarea type="text" class="form-control form-control-sm" id="Vketerangan"
                                disabled></textarea>
                        </div>
                    </div>
                </div>
                ...
                <div class="card border-primary">
                    ...
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table VtableOrder" id="VtableOrder">
                                <thead>
                                    <tr>
                                        <th>Kode</th>
                                        <th>Nama</th>
...
<script>
    ...
    $('#tablePermintaan').on('click', '.validasiobat', function () {
        $('#VtableOrder tbody').html('');
        $('#VnoPerimtaaan').val($(this).data('id'))
        $('#VtglPerimtaaan').val($(this).data('tgl'))
        $('#Vkdtujuan').val($(this).data('tujuan'))
        $('#Vtujuan').val($(this).data('nmtujuan'))
        $('#Vkddari').val($(this).data('dari'))
        $('#Vdari').val($(this).data('nmdari'))
        $('#Vketerangan').val($(this).data('keterangan'))

        $.ajax({
            url: "/permintaan/getitempermintaanobat",
            type: 'POST',
            data: {
                id: $(this).data('id')
            },
                ...
                if (response.code == 200) {
                    if (response.list_obat) {
                        ...
                        for (let i = 0; i < response.list_obat.length; i++) {
                            var row = '';
                            row += '<tr ';
                            row += 'data-kodebarang="' + response.list_obat[i][0];
                            row += '" data-namabrng="' + response.list_obat[i][1];
                            row += '" data-stokunit="' + response.list_obat[i][2];
                            row += '" data-stoktujuan="' + response.list_obat[i][3];
                            row += '" data-jumlah="' + response.list_obat[i][4];
                            row += '">';

                            row += '<td>' + response.list_obat[i][0] + '</td>';
                            row += '<td>' + response.list_obat[i][1] + '</td>';
                            row += '<td>' + response.list_obat[i][2] + '</td>';
                            row += '<td>' + response.list_obat[i][3] + '</td>';
                            row += '<td>' + response.list_obat[i][4] + '</td>';
                            row += '<td>';
                            row +=
        ...
        $('#modalValidasiPermintaan').modal('show')
    });
    ...
    $('#btnsavepermintaan').click(function (e) {
        Swal.fire({
            target: document.getElementById('modalAddObat'),
            ...
            if (result.isConfirmed) {
                ...
                if (arraylistitem.length > 0) {
                    $.ajax({
                        url: "/permintaan/postPermintaan",
                        type: 'POST',
                        data: {

                            tgl_permintaan: $("#tglPerimtaaan").val(),
                            dari: $('#dari').val(),
                            tujuan: $('#tujuan').val(),
                        ...
                        success: function (response) {
                            ...
                            if (response.code == 200) {
                                ...
                                Swal.fire({
                                    title: "Gagal",
                                    message: "Periksa Kembali data / Silahkan Hubungi SIMRS",
                                    icon: "error",
                                    showConfirmButton: false,
                                    timer: 1000
                                })
                    ...
                    Swal.fire({
                        target: document.getElementById('modalAddPermintaan'),
    ...
    $('#btnvalidasipermintaan').click(function (e) {
        Swal.fire({
            target: document.getElementById('modalValidasiPermintaan'),
            title: 'Apa Anda Yakin?',
            text: "Periksa Kembali, Data Permintaan Obat yang sudah di input tidak bisa diubah maupun dihapus",
            icon: 'warning',
            showCancelButton: true,
            ...
            if (result.isConfirmed) {
                ...
                arraylistitem = [];
                $("table.VtableOrder tbody tr").each(function (i) {
                    var arrayval = [];
                    // var key = ['kode', 'nama', 'stokasal', 'stoktujuan', 'jumlah', 'aksi'];              
                    $("td", this).each(function (j) {
                        arrayval.push($(this).text())
                ...
                if (arraylistitem.length > 0) {
                    $.ajax({
                        ...
                        data: {
                            ...
                            id: $("#VnoPerimtaaan").val(),
                            tanggal: $("#VtglPerimtaaan").val(),
                            kd_bangsalke: $('#Vkddari').val(),
                            kd_bangsaldari: $('#Vkdtujuan').val(),
                            keterangan: $("#Vketerangan").val(),
                        ...
                        success: function (response) {
                            ...
                            if (response.code == 200) {
                                // Swal.close();
                                $('#modalValidasiPermintaan').modal('hide')
                                Swal.fire({
                                    title: "Berhasil",
                                    icon: "success",
                                    showConfirmButton: false,
